...
export const AssetsLiabilities: React.FC<Props> = ({ data, onUpdate }) => {
  ...
  const addAsset = () => {
    const newAsset: Asset = {
      ...
      value: 0,
      type: 'CASH',
      growthRate: 0.06 // 6% conservative default
    };
  ...
  const addLiability = () => {
    const newLiab: Liability = {
      ...
      value: 0,
      type: 'PERSONAL_LOAN',
      interestRate: 0.10 // 10% typical personal loan
    };
          ...
          {activeTab === 'assets' && (
            <>
              {data.assets.map(asset => (
                <div key={asset.id} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center bg-white/5 p-4 rounded-xl border border-white/5 hover:border-white/20 transition-all">
                  ...
                  <div className="sm:col-span-1 flex justify-end">
                    <button onClick={() => removeAsset(asset.id)} className="p-2 text-slate-500 hover:text-rose-400 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ))}
              <button onClick={addAsset} className="w-full py-3 border border-dashed border-slate-600 rounded-xl text-slate-400 hover:text-white hover:border-emerald-400 hover:bg-emerald-400/5 transition-all flex items-center justify-center gap-2">
          ...
          {activeTab === 'liabilities' && (
            <>
              {data.liabilities.map(liab => (
                <div key={liab.id} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center bg-white/5 p-4 rounded-xl border border-white/5 hover:border-white/20 transition-all">
                  <div className="sm:col-span-4">
                    <label className="text-xs text-slate-400 block mb-1">Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="sm:col-span-1 flex justify-end">
                    <button onClick={() => removeLiability(liab.id)} className="p-2 text-slate-500 hover:text-rose-400 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ))}
              <button onClick={addLiability} className="w-full py-3 border border-dashed border-slate-600 rounded-xl text-slate-400 hover:text-white hover:border-rose-400 hover:bg-rose-400/5 transition-all flex items-center justify-center gap-2">
